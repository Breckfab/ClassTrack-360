...
if st.session_state.user is None:
        ...
        if df_cursos.empty:
            ...
            try:
                ...
                if res_b.data:
                    ...
                    if tarea_p: st.markdown(f'<div class="reminder-box">🔔 <b>Tarea para revisar hoy:</b><br>{tarea_p}</div>', unsafe_allow_html=True)
                    else: st.markdown('<div class="reminder-box">✅ No hay tarea para hoy.</div>', unsafe_allow_html=True)
                    st.info(f"📍 Clase anterior: {res_b.data[0].get('temas_dictados', 'Sin registro')}")
                else: st.markdown('<div class="reminder-box">📝 No hay registros previos.</div>', unsafe_allow_html=True)
            except: pass
            
            with st.form("f_agenda_v3"):
                temas = st.text_area("Temas de hoy")
                tarea_n = st.text_area("Tarea para la próxima")
                if st.form_submit_button("Guardar Clase"):
                    if temas:
                        try:
                            supabase.table("bitacora").insert({"profesor_id": user['id'], "materia": c_agenda, "fecha": str(hoy.date()), "temas_dictados": temas, "tarea_proxima": tarea_n}).execute()
                            st.success("Guardado."); st.rerun()
                        except: st.error("Error al guardar.")

    # --- TAB 1: ALUMNOS (CONTADORES Y LISTA) ---
    with tabs[1]:
    ...
    # --- TAB 2 Y 3: ASISTENCIA Y NOTAS (LEYENDAS CORREGIDAS) ---
