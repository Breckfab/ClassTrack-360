...
if st.session_state.user is None:
    ...
    with tabs[0]:
        ...
        if df_cursos.empty: st.markdown('<div class="info-card">🏗️ No hay materias creadas para registrar agenda.</div>', unsafe_allow_html=True)
            ...
            m_age = st.selectbox("Materia:", df_cursos['nombre_curso_materia'].unique(), key="sb_age_v47")
            c1, c2 = st.columns(2)
            with c1:
                with st.form("f_age_v47", clear_on_submit=True):
                    ...
                    if st.form_submit_button("Guardar"):
                        if t1:
                            try:
                                ...
                                data_ins = {"profesor_id": u_data['id'], "materia": m_age, "fecha": str(ahora.date()), "temas_dictados": t1, "tarea_proxima": txt_tarea}
                                supabase.table("bitacora").insert(data_ins).execute()
                                st.success("✅ Clase registrada."); st.rerun()
                            except:
                                st.error("❌ No se pudo guardar en 'bitacora'. Verificá que la tabla exista en Supabase.")
            with c2:
                    ...
                    if r_h and r_h.data:
                        for entry in r_h.data:
                            with st.expander(f"📅 {entry['fecha']}"):
                                ...
                                if st.button("Editar Registro", key=f"ed_age_{entry['id']}"):
    ...
    with tabs[1]:
        ...
        if df_cursos.empty: st.markdown('<div class="info-card">⚠️ No hay materias creadas para gestionar alumnos.</div>', unsafe_allow_html=True)
        else:
            m_alu = st.selectbox("Materia:", df_cursos['nombre_curso_materia'].unique(), key="sb_alu_v47")
                ...
                for x in r_alu.data:
                    if x.get('alumnos'):
                        ...
                        with st.expander(f"👤 {alu.get('apellido')}, {alu.get('nombre')}"):
                            if st.button("Baja Alumno", key=f"bj_v47_{x['id']}"):
            ...
            with st.expander("➕ Inscribir Alumno"):
                with st.form("f_ins_v47", clear_on_submit=True):
    ...
    with tabs[2]:
        ...
        if df_cursos.empty: st.markdown('<div class="info-card">⚠️ Debes crear una materia para el control de asistencia.</div>', unsafe_allow_html=True)
        else:
            m_as = st.selectbox("Materia:", df_cursos['nombre_curso_materia'].unique(), key="sb_as_v47")
            sub_as = st.tabs(["📝 Tomar", "📊 Por Fecha", "🔍 Por Alumno"])
            with sub_as[0]:
                ...
                if not r_as or not r_as.data: st.info("ℹ️ No hay alumnos inscriptos.")
                    ...
                    with st.form("f_as_v47"):
                        as_list = []
                        for it in r_as.data:
                            alu = it['alumnos']
                            est = st.radio(f"{alu['apellido']}", ["Presente", "Ausente"], key=f"as_v47_{alu['id']}", horizontal=True)
            ...
            with sub_as[1]:
                fq = st.date_input("Fecha:", value=ahora.date(), key="fq_as_v47")
                try:
            ...
            with sub_as[2]:
                r_al = supabase.table("inscripciones").select("alumnos(id, nombre, apellido)").eq("nombre_curso_materia", m_as).not_.is_("alumno_id", "null").execute()
                if r_al and r_al.data:
                    op = {f"{it['alumnos']['apellido']}, {it['alumnos']['nombre']}": it['alumnos']['id'] for it in r_al.data}
                    sel = st.selectbox("Alumno:", list(op.keys()), key="sel_as_v47")
                    if st.button("Buscar", key="btn_as_v47"):
                        id_a = op[sel]
                        rh = supabase.table("asistencia").select("id, fecha, estado").eq("alumno_id", id_a).eq("materia", m_as).order("fecha", desc=True).execute()
                        if rh and rh.data:
                            for h in rh.data: st.write(f"📅 {h['fecha']}: {h['estado']}")
                        else: st.info("ℹ️ No hay alumnos para consultar registros de asistencia.")
                else: st.info("ℹ️ No hay alumnos inscriptos para buscar registros.")

    # --- TAB 3: NOTAS ---
    with tabs[3]:
        st.subheader("Notas")
        if df_cursos.empty: st.markdown('<div class="info-card">⚠️ Debes crear una materia para la carga de notas.</div>', unsafe_allow_html=True)
        else:
            m_nt = st.selectbox("Materia:", df_cursos['nombre_curso_materia'].unique(), key="sb_nt_v47")
            sub_nt = st.tabs(["📝 Cargar", "🔍 Por Alumno"])
            with sub_nt[0]:
                ...
                if r_nt and r_nt.data:
                    with st.form("f_nt_v47", clear_on_submit=True):
                        inst = st.selectbox("Instancia", ["Parcial 1", "Parcial 2", "TP", "Final"])
                        nl = []
                        for it in r_nt.data:
                            alu = it['alumnos']
                            v = st.number_input(f"{alu['apellido']}", 1, 10, key=f"n_v47_{alu['id']}")
            ...
            with sub_nt[1]:
                ...
                if r_al_n and r_al_n.data:
                    ...
                    sel_n = st.selectbox("Alumno:", list(op_n.keys()), key="sel_nt_v47")
                    if st.button("Buscar", key="btn_nt_v47"):
                        id_a_n = op_n[sel_n]
