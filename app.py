...
if st.session_state.user is None:
    ...
    with col2:
        ...
        with st.form("login_form_v17"):
            u_input = st.text_input("Usuario", placeholder="Sede (ej. Cambridge)").strip().lower()
    ...
    with tabs[0]:
        ...
        if df_cursos.empty:
            ...
            c_agenda = st.selectbox("Seleccione Materia:", df_cursos['nombre_curso_materia'].unique())
            tarea_ant = ""
            try:
                ...
                if res_b and res_b.data:
                    tarea_ant = res_b.data[0].get("tarea_proxima", "")
            except:
                pass

            if tarea_ant:
                st.markdown(f'<div class="reminder-box">🔔 <b>Tarea para Hoy:</b><br>{tarea_ant}</div>', unsafe_allow_html=True)
            else:
                st.markdown('<div class="reminder-box">✅ <b>No hay tarea pendiente.</b></div>', unsafe_allow_html=True)

            with st.form("form_agenda_v17"):
                t_hoy = st.text_area("Temas dictados hoy")
                t_prox = st.text_area("Tarea para la próxima")
                f_prox = st.date_input("Fecha entrega:", value=ahora_dt + datetime.timedelta(days=7))
    ...
    with tabs[1]:
        ...
        if df_cursos.empty:
            ...
            try:
                ...
                st.markdown(f'<div class="metric-card">Matrícula Total 2026: <span class="metric-value">{res_t.count if res_t and res_t.count else 0}</span></div>', unsafe_allow_html=True)
                
                sel_m = st.selectbox("Filtrar por materia:", df_cursos['nombre_curso_materia'].unique(), key="sel_alu_v17")
                res_alu = supabase.table("inscripciones").select("alumnos(nombre, apellido)").eq("profesor_id", user['id']).eq("nombre_curso_materia", sel_m).not_.is_("alumno_id", "null").execute()
                if res_alu and res_alu.data:
                    for a in res_alu.data:
                        if a and a.get('alumnos'):
