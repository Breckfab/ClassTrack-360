...
if st.session_state.user is None:
    ...
    with tabs[0]:
        ...
        if df_cursos.empty: st.info("🏗️ Crea una materia primero.")
            ...
            m_age = st.selectbox("Materia:", df_cursos['nombre_curso_materia'].unique(), key="sb_age_v34")
            c1, c2 = st.columns(2)
            with c1:
                with st.form("f_age_v34"):
    ...
    with tabs[1]:
        ...
        if df_cursos.empty: st.warning("Crea una materia primero.")
            ...
            m_alu = st.selectbox("Materia:", df_cursos['nombre_curso_materia'].unique(), key="sb_alu_v34")
    ...
    with tabs[2]:
        ...
        if df_cursos.empty: st.warning("Crea una materia.")
            ...
            m_as = st.selectbox("Materia:", df_cursos['nombre_curso_materia'].unique(), key="sb_as_v34")
            ...
            with sub_asis[0]:
                ...
                if not r_as or not r_as.data: st.info("Sin alumnos inscriptos.")
                    ...
                    with st.form("f_as_v34"):
                        as_list = []
                        for it in r_as.data:
                            if it and it.get('alumnos'):
                                alu = it['alumnos']
                                est = st.radio(f"{alu['apellido']}", ["Presente", "Ausente"], key=f"as_v34_{alu['id']}", horizontal=True)
    ...
    with tabs[3]:
        ...
        if df_cursos.empty: st.warning("Crea una materia.")
            ...
            m_nt = st.selectbox("Materia:", df_cursos['nombre_curso_materia'].unique(), key="sb_nt_v34")
            ...
            if r_nt and r_nt.data:
                with st.form("f_nt_v34"):
                    inst = st.selectbox("Instancia", ["Parcial 1", "Parcial 2", "TP", "Final"])
                    nt_list = []
                    for it in r_nt.data:
                        alu = it['alumnos']
                        v = st.number_input(f"{alu['apellido']}", 1, 10, key=f"n_v34_{alu['id']}")
    ...
    with tabs[4]:
        ...
        if not df_cursos.empty:
            for _, r in df_cursos.iterrows():
                ...
                if c2.button("Borrar", key=f"br_v34_{r['id']}"):
                    supabase.table("inscripciones").delete().eq("id", r['id']).execute()
                    st.rerun()
        with st.form("f_cur_v34"):
