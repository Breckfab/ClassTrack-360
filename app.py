...
st.markdown("""
    ...
    .avg-badge { background: #3b82f6; color: white; padding: 2px 8px; border-radius: 12px; font-size: 0.85rem; font-weight: bold; margin-left: 10px; }
    </style>
...
if st.session_state.user is None:
    ...
    with tabs[0]:
        ...
        if df_cursos.empty: st.info("🏗️ Crea una materia primero.")
            ...
            m_age = st.selectbox("Materia:", df_cursos['nombre_curso_materia'].unique(), key="sb_age_v37")
            c1, c2 = st.columns(2)
            with c1:
                with st.form("f_age_v37", clear_on_submit=True):
    ...
    # --- TAB 1: ALUMNOS (PROMEDIO INTEGRADO) ---
    with tabs[1]:
        st.subheader("Alumnos")
        if df_cursos.empty: st.warning("Crea una materia primero.")
        else:
            m_alu = st.selectbox("Materia:", df_cursos['nombre_curso_materia'].unique(), key="sb_alu_v37")
            ...
            if r_alu and r_alu.data:
                for x in r_alu.data:
                    if x.get('alumnos'):
                        ...
                        # Cálculo de promedio seguro (sin error si no hay notas)
                        promedio_texto = "Sin notas"
                        try:
                            r_notas = supabase.table("notas").select("calificacion").eq("alumno_id", alu['id']).eq("materia", m_alu).execute()
                            if r_notas and r_notas.data:
                                vals = [n['calificacion'] for n in r_notas.data]
                                promedio_texto = f"PROM: {round(sum(vals)/len(vals), 2)}"
                        except: pass

                        with st.expander(f"👤 {alu.get('apellido')}, {alu.get('nombre')} 🏷️ {promedio_texto}"):
                            if st.button("Baja", key=f"bj_{x['id']}"):
                                supabase.table("inscripciones").delete().eq("id", x['id']).execute()
                                st.rerun()
            else: st.info("No hay alumnos inscriptos en esta materia.")

            with st.expander("➕ Inscribir Alumno"):
                with st.form("f_ins_v37", clear_on_submit=True):
    ...
    with tabs[2]:
        ...
        if df_cursos.empty: st.warning("Crea una materia.")
            ...
            m_as = st.selectbox("Materia:", df_cursos['nombre_curso_materia'].unique(), key="sb_as_v37")
            ...
            with sub_asis[0]:
                ...
                if not r_as or not r_as.data: st.info("Sin alumnos inscriptos.")
                    ...
                    with st.form("f_as_v37"):
                        as_list = []
                        for it in r_as.data:
                            if it and it.get('alumnos'):
                                alu = it['alumnos']
                                est = st.radio(f"{alu['apellido']}", ["Presente", "Ausente"], key=f"as_v37_{alu['id']}", horizontal=True)
    ...
    with tabs[3]:
        ...
        if df_cursos.empty: st.warning("Crea una materia.")
            ...
            m_nt = st.selectbox("Materia:", df_cursos['nombre_curso_materia'].unique(), key="sb_nt_v37")
            ...
            if r_nt and r_nt.data:
                with st.form("f_nt_v37", clear_on_submit=True):
                    inst = st.selectbox("Instancia", ["Parcial 1", "Parcial 2", "TP", "Final"])
                    nt_list = []
                    for it in r_nt.data:
                        alu = it['alumnos']
                        v = st.number_input(f"{alu['apellido']}", 1, 10, key=f"n_v37_{alu['id']}")
    ...
    with tabs[4]:
        ...
        if not df_cursos.empty:
            for _, r in df_cursos.iterrows():
                ...
                if c2.button("Borrar", key=f"br_v37_{r['id']}"):
                    supabase.table("inscripciones").delete().eq("id", r['id']).execute()
                    st.rerun()
        
        with st.form("f_cur_v37", clear_on_submit=True):
