...
if st.session_state.user is None:
    ...
    # --- TAB 0: AGENDA (CON FLUJO PEDAGÓGICO COMPLETO) ---
    with tabs[0]:
        ...
        if df_cursos.empty:
            ...
            # Recuperar Tarea que dejaste la clase anterior
            tarea_de_antes = ""
            temas_anteriores = ""
            try:
                res_b = supabase.table("bitacora").select("*").eq("profesor_id", user['id']).eq("materia", c_agenda).order("fecha", desc=True).limit(1).execute()
                if res_b.data:
                    tarea_de_antes = res_b.data[0].get("tarea_proxima", "")
                    temas_anteriores = res_b.data[0].get("temas_dictados", "Sin registro")
            except: pass

            if tarea_de_antes:
                st.markdown(f'<div class="reminder-box">🔔 <b>Tarea para Hoy (traída de la clase anterior):</b><br>{tarea_de_antes}</div>', unsafe_allow_html=True)
            else:
                st.markdown('<div class="reminder-box">✅ <b>No hay tarea para revisar hoy.</b></div>', unsafe_allow_html=True)
            
            if temas_anteriores:
                st.info(f"📍 En la última clase viste: {temas_anteriores}")

            st.write("---")
            with st.form("form_agenda_completo"):
