...
# --- RELOJ SUPERIOR DERECHO (INYECCIÓN IFRAME) ---
st.markdown("""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;700;800&family=JetBrains+Mono&display=swap');
    .stApp { background: linear-gradient(135deg, #0b0e14 0%, #1e293b 100%); color: #e0e0e0; font-family: 'Inter', sans-serif; }
    .logo-text { font-size: 2.5rem; font-weight: 800; background: linear-gradient(to right, #4facfe, #00f2fe); -webkit-background-clip: text; -webkit-fill-color: transparent; text-align: center; margin-bottom: 20px; }
    </style>
    """, unsafe_allow_html=True)

# Componente de reloj blanco, discreto y automático
components.html("""
    <div id="clock" style="
        position: fixed;
        top: 5px;
...
if st.session_state.user is None:
    ...
    # --- TAB 0: AGENDA (CALENDARIO RESTAURADO) ---
    with tabs[0]:
        st.subheader("Registro de Clase")
        if mapa_cursos:
            m_age = st.selectbox("Curso:", ["--- Seleccionar ---"] + list(mapa_cursos.keys()), key="sb_age_v113")
            if m_age != "--- Seleccionar ---":
                with st.form("f_age_v113"):
                    t1 = st.text_area("Dictado")
