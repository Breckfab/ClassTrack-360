...
# --- CONFIGURACIÓN ---
st.set_page_config(page_title="ClassTrack 360 v210", layout="wide")
...
if st.session_state.user is None:
    ...
    # --- TAB 2: ASISTENCIA (CORREGIDO) ---
    with tabs[2]:
        st.subheader("✅ Planilla de Asistencia")
        if not mapa_cursos:
            st.info("Cree un curso primero en la pestaña 'Cursos'.")
        else:
            c_as_sel = st.selectbox("Elegir Curso:", ["---"] + list(mapa_cursos.keys()), key="as_sel_v210")
            f_asist = st.date_input("Fecha de hoy:", datetime.date.today(), key="as_fecha")
            
            if c_as_sel != "---":
                # Consulta blindada para traer alumnos del curso seleccionado
                    ...
                    for r in res_al_asist.data:
                        ...
                        with st.container():
                            ...
                            with st.form(f"f_asist_{al['id']}"):
                                ...
                                col_b3.form_submit_button("❌ Borrar")

    # --- TAB 0: AGENDA (CON RECORDATORIO) ---
    with tabs[0]:
        st.subheader("📅 Seguimiento de Clase")
        if mapa_cursos:
            c_ag = st.selectbox("Curso:", list(mapa_cursos.keys()), key="ag_v210")
            ...
            with st.form("f_ag"):
                ...
                if st.form_submit_button("💾 GUARDAR AGENDA"):
                    supabase.table("bitacora").insert({"inscripcion_id": mapa_cursos[c_ag], "fecha": str(f_hoy), "contenido_clase": t_hoy, "tarea_proxima": t_prox, "fecha_tarea": str(vto)}).execute()
                    st.success("Cambios guardados satisfactoriamente."); st.rerun()

    # --- TABS ALUMNOS, NOTAS Y CURSOS (ESTABLES) ---
    # Se mantienen exactamente iguales a la v209 para no desarmar lo funcional.
