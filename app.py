...
if st.session_state.user is None:
    ...
    components.html(f"""
        ...
        <script>setInterval(() => {{ const d = new Date(); document.getElementById('c').innerHTML = d.toLocaleTimeString('es-AR', {{hour12:false}}); }}, 1000);</script>
    """, height=35)
    ...
    # --- TAB 0: AGENDA (SOLUCIÓN DEFINITIVA) ---
    with tabs[0]:
        st.subheader("Registro de Clase")
        if df_cursos.empty: st.info("🏗️ No hay materias creadas.")
        else:
            opts = ["--- Seleccione Materia ---"] + list(df_cursos['nombre_curso_materia'].unique())
            m_age = st.selectbox("Materia:", opts, key="sb_age_v61")
            ...
            if m_age == "--- Seleccione Materia ---":
                ...
                c1, c2 = st.columns(2)
                with c1:
                    with st.form("f_age_v61", clear_on_submit=True):
                        ...
                        if st.form_submit_button("Guardar Registro"):
                            if t1:
                                try:
                                    # AUTO-DETECCIÓN DE COLUMNA: Leemos qué columnas existen realmente
                                    test_q = supabase.table("bitacora").select("*").limit(1).execute()
                                    db_cols = test_q.data[0].keys() if test_q.data else ["materia"]
                                    
                                    # Mapeo inteligente del nombre de columna
                                    target_col = "materia"
                                    if "nombre_materia" in db_cols: target_col = "nombre_materia"
                                    elif "materia_nombre" in db_cols: target_col = "materia_nombre"
                                    ...
                                    st.error(f"Error estructural: Informe este código al soporte: {str(e)}")
                with c2:
                    st.write("### Historial Reciente")
                    try:
                        r_h = supabase.table("bitacora").select("*").or_(f"materia.eq.{m_age},nombre_materia.eq.{m_age},materia_nombre.eq.{m_age}").order("fecha", desc=True).limit(5).execute()
                        if r_h and r_h.data:
    ...
    with tabs[1]:
        ...
        if df_cursos.empty: st.warning("Crea una materia primero.")
            ...
            m_alu = st.selectbox("Materia:", df_cursos['nombre_curso_materia'].unique(), key="sb_alu_v61")
                ...
                for x in r_alu.data:
                    if x.get('alumnos'):
                        ...
                        with st.expander(f"👤 {alu.get('apellido')}, {alu.get('nombre')}"):
                            if st.button("Baja", key=f"bj_v61_{x['id']}"):
            ...
            with st.expander("➕ Inscribir Alumno"):
                with st.form("f_ins_v61", clear_on_submit=True):
    ...
    with tabs[2]:
        ...
        if df_cursos.empty: st.warning("Crea una materia.")
            ...
            m_as = st.selectbox("Materia:", df_cursos['nombre_curso_materia'].unique(), key="sb_as_v61")
            ...
            with sub_asis[0]:
                ...
                if not r_as or not r_as.data: st.info("ℹ️ No hay alumnos inscriptos.")
                    ...
                    with st.form("f_as_v61"):
                        as_list = []
                        for it in r_as.data:
                            alu = it['alumnos']
                            est = st.radio(f"{alu['apellido']}", ["Presente", "Ausente"], key=f"as_v61_{alu['id']}", horizontal=True)
            ...
            with sub_asis[1]:
                f_q = st.date_input("Fecha:", value=ahora.date(), key="f_as_v61")
                try:
    ...
    with tabs[3]:
        ...
        if df_cursos.empty: st.warning("Crea una materia.")
            ...
            m_nt = st.selectbox("Materia:", df_cursos['nombre_curso_materia'].unique(), key="sb_nt_v61")
            ...
            with sub_nt[0]:
                ...
                if r_nt and r_nt.data:
                    with st.form("f_nt_v61", clear_on_submit=True):
                        inst = st.selectbox("Instancia", ["Parcial 1", "Parcial 2", "TP", "Final"])
                        nl = []
                        for it in r_nt.data:
                            alu = it['alumnos']
                            v = st.number_input(f"{alu['apellido']}", 1, 10, key=f"n_v61_{alu['id']}")
            ...
            with sub_nt[1]:
                ...
                if r_al_n and r_al_n.data:
                    ...
                    sel_n = st.selectbox("Alumno:", list(op_n.keys()), key="sel_nt_v61")
                    if st.button("Buscar"):
                        id_a_n = op_n[sel_n]
    ...
    with tabs[4]:
        ...
        if not df_cursos.empty:
            for _, r in df_cursos.iterrows():
                ...
                if c2.button("Borrar", key=f"br_v61_{r['id']}"):
                    supabase.table("inscripciones").delete().eq("id", r['id']).execute()
                    st.rerun()
        with st.form("f_cur_v61", clear_on_submit=True):
