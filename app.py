...
if st.session_state.user is None:
    ...
    with tabs[0]:
        ...
        if df_cursos.empty:
            ...
            st.write("---")
            with st.form("form_agenda_completo_v2"):
                ...
                if st.form_submit_button("Guardar Clase"):
                    if t_hoy:
                        ...
                        st.warning("Completa los temas dictados hoy.")
    ...
    with tabs[1]:
        ...
        if df_cursos.empty:
            ...
            try:
                ...
                mat_f = st.selectbox("Ver alumnos de:", df_cursos['nombre_curso_materia'].unique(), key="sel_alu_tab_v2")
    ...
    with tabs[2]:
        ...
        if df_cursos.empty:
            ...
            mat_asis = st.selectbox("Curso:", df_cursos['nombre_curso_materia'].unique(), key="asis_mat_v13")
            try:
                ...
                if not res_a or not res_a.data:
                    st.markdown('<div class="warning-card">👤 No hay alumnos inscritos para tomar asistencia.</div>', unsafe_allow_html=True)
                else:
                    st.write(f"Fecha: {ahora_dt.strftime('%d/%m/%Y')}")
                    with st.form("form_asis_v13"):
                        for item in res_a.data:
                            if item and 'alumnos' in item and item['alumnos']:
                                ...
                                col_a2.radio("Estado:", ["Presente", "Ausente"], key=f"a_radio_{a_id}", horizontal=True)
                        if st.form_submit_button("Guardar Asistencia"):
                            st.success("Asistencia registrada.")
            except:
                st.error("Error en asistencia.")
    ...
    with tabs[3]:
        ...
        if df_cursos.empty:
            ...
            mat_n = st.selectbox("Curso:", df_cursos['nombre_curso_materia'].unique(), key="notas_mat_v13")
            try:
                ...
                if not res_n or not res_n.data:
                    ...
                    st.markdown('<div class="warning-card">📝 <b>No hay notas para mostrar porque no se registraron aún.</b></div>', unsafe_allow_html=True)
                    with st.expander("📝 Cargar Calificaciones"):
                        with st.form("form_notas_v13"):
                            for item in res_n.data:
                                if item and 'alumnos' in item and item['alumnos']:
                                    alu = item['alumnos']
                                    a_id = alu.get('id', 'desconocido')
                                    st.number_input(f"{alu.get('apellido', 'N/A')}, {alu.get('nombre', 'N/A')}", min_value=1, max_value=10, step=1, key=f"n_val_{a_id}")
                            if st.form_submit_button("Guardar Notas"):
                                st.success("Notas guardadas.")
            except:
                st.error("Error en notas.")
