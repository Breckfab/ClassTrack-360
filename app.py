...
# --- 1. NÚCLEO ---
st.set_page_config(page_title="ClassTrack 360 v242", layout="wide")
...
if st.session_state.user is None:
    ...
    # --- TAB 0: AGENDA (SENSOR DE TAREA) ---
    with tabs[0]:
        st.subheader("📅 Gestión de Agenda")
        if mapa_cursos:
            c_ag = st.selectbox("Curso:", list(mapa_cursos.keys()), key="ag_v242")
            f_hoy = st.date_input("Fecha:", datetime.date.today())
            # Busca tarea cuya fecha de entrega sea hoy
            ...
            with st.form("f_ag_v242"):
                temas, tarea_p = st.text_area("Temas dictados hoy"), st.text_area("Nueva tarea")
                vto = st.date_input("Vencimiento:", f_hoy + datetime.timedelta(days=7))
                b1, b2, b3, _ = st.columns([1,1,1,5])
                if b1.form_submit_button("💾 Guardar"):
                    supabase.table("bitacora").insert({"inscripcion_id": mapa_cursos[c_ag], "fecha": str(f_hoy), "contenido_clase": temas, "tarea_proxima": tarea_p, "fecha_tarea": str(vto)}).execute()
    ...
    with tabs[1]:
        ...
        if sub_al == "Nuevo Alumno":
            ...
            c_v = st.selectbox("Curso:", ["---"] + list(mapa_cursos.keys()), key="al_list_v242")
            if c_v != "---":
    ...
    with tabs[2]:
        ...
        sub_as = st.radio("Acción:", ["Tomar Asistencia", "Consultar Asistencia"], horizontal=True, key="as_rad")
        c_as = st.selectbox("Curso:", ["---"] + list(mapa_cursos.keys()), key="as_v242")
