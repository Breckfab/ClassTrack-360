...
if st.session_state.user is None:
    ...
    # --- TAB 0: AGENDA (CON VALIDACIÓN DE ESCRITURA) ---
    with tabs[0]:
        ...
        if df_cursos.empty: st.markdown('<div class="info-card">🏗️ No hay materias creadas para registrar agenda.</div>', unsafe_allow_html=True)
            ...
            m_age = st.selectbox("Materia:", df_cursos['nombre_curso_materia'].unique(), key="sb_age_v46")
            c1, c2 = st.columns(2)
            with c1:
                with st.form("f_age_v46", clear_on_submit=True):
                    ...
                    if st.form_submit_button("Guardar"):
                        if not t1:
                            st.warning("⚠️ El campo 'Temas dictados hoy' no puede estar vacío.")
                        else:
                            try:
                                ...
                                supabase.table("bitacora").insert(data_bit).execute()
                                st.success("✅ Registro guardado correctamente.")
                                st.rerun()
                            except Exception as e:
                                st.error(f"❌ Error al guardar: No se pudo escribir en la tabla 'bitacora'. Verifica los permisos o el esquema de la base de datos.")
            with c2:
    ...
    with tabs[1]:
        ...
        if df_cursos.empty: st.markdown('<div class="info-card">⚠️ Crea una materia primero para gestionar alumnos.</div>', unsafe_allow_html=True)
            ...
            m_alu = st.selectbox("Materia:", df_cursos['nombre_curso_materia'].unique(), key="sb_alu_v46")
                    ...
                    if x.get('alumnos'):
                        ...
                        with st.expander(f"👤 {alu.get('apellido')}, {alu.get('nombre')}"):
                            if st.button("Baja", key=f"bj_{x['id']}"):
                                ...
                                st.rerun()
            else: st.info("ℹ️ No hay alumnos inscriptos para consultar en esta materia.")
            with st.expander("➕ Inscribir Alumno"):
                with st.form("f_ins_v46", clear_on_submit=True):
    ...
    with tabs[2]:
        ...
        if df_cursos.empty: st.markdown('<div class="info-card">⚠️ Debes crear una materia para habilitar el control de asistencia.</div>', unsafe_allow_html=True)
            ...
            m_as = st.selectbox("Materia:", df_cursos['nombre_curso_materia'].unique(), key="sb_as_v46")
            ...
            with sub_asis[0]:
                ...
                if not r_as or not r_as.data: st.info("ℹ️ No hay alumnos inscriptos para tomar asistencia.")
                else:
                    with st.form("f_as_v46"):
                        as_list = []
                        for it in r_as.data:
                            alu = it['alumnos']
                            est = st.radio(f"{alu['apellido']}", ["Presente", "Ausente"], key=f"as_v46_{alu['id']}", horizontal=True)
            ...
            with sub_asis[1]:
                f_q = st.date_input("Elegir fecha:", value=ahora.date(), key="f_as_v46")
                try:
                    ...
                    else: st.info("ℹ️ No hay registros de asistencia para esta fecha.")
            ...
            with sub_asis[2]:
                ...
                if r_al_as and r_al_as.data:
                    ...
                    sel_as = st.selectbox("Seleccionar Alumno:", list(op_as.keys()), key="sel_as_v46")
                    id_al = op_as[sel_as]
    ...
    with tabs[3]:
        ...
        if df_cursos.empty: st.markdown('<div class="info-card">⚠️ Debes crear una materia para habilitar la carga de notas.</div>', unsafe_allow_html=True)
            ...
            m_nt = st.selectbox("Materia:", df_cursos['nombre_curso_materia'].unique(), key="sb_nt_v46")
            ...
            with sub_nt[0]:
                ...
                if r_nt and r_nt.data:
                    with st.form("f_nt_v46", clear_on_submit=True):
                        inst = st.selectbox("Instancia", ["Parcial 1", "Parcial 2", "TP", "Final"])
                        nt_list = []
                        for it in r_nt.data:
                            alu = it['alumnos']
                            v = st.number_input(f"{alu['apellido']}", 1, 10, key=f"n_v46_{alu['id']}")
            ...
            with sub_nt[1]:
                ...
                if r_al_nt and r_al_nt.data:
                    ...
                    sel_nt = st.selectbox("Alumno:", list(op_nt.keys()), key="sel_nt_v46")
